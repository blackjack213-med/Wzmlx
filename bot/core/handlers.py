...
async def add_handlers():
    ...
    TgClient.bot.add_handler(
        MessageHandler(
            user_session,
            filters=command(BotCommands.UserSessionCommand, case_sensitive=True)
            & CustomFilters.sudo,
        )
    )
    TgClient.bot.add_handler(
        MessageHandler(
            topic_clone,
            filters=command(BotCommands.TopicCloneCommand, case_sensitive=True)
            & CustomFilters.owner,
        )
    )
    TgClient.bot.add_handler(
        MessageHandler(
            promo,
            filters=command(BotCommands.PromoCommand, case_sensitive=True)
            & CustomFilters.owner,
        )
    )
    TgClient.bot.add_handler(
