...
class TgClient:
    ...
    @classmethod
    async def get_personal_client(cls, user_id, session_string=None):
        """Return a cached personal client for user_id, or start one from
        session_string (validating it in the process). Raises on failure
        so the caller (e.g. the /usersession command) can report the error."""
        existing = cls.personal_users.get(user_id)
        if existing is not None:
            return existing
        if not session_string:
            return None
        async with cls._plock:
            existing = cls.personal_users.get(user_id)
            if existing is not None:
                return existing
            client = cls.wztgClient(
                f"WZ-Personal{user_id}",
                session_string=session_string,
                sleep_threshold=60,
                no_updates=True,
            )
    ...
    @classmethod
    async def remove_personal_client(cls, user_id):
        async with cls._plock:
    ...
    async def stop(cls):
        async with cls._lock:
            ...
            if cls.helper_users:
                ...
                cls.helper_users = {}
            if cls.personal_users:
                clients.extend(p.stop() for p in cls.personal_users.values())
                cls.personal_users = {}
            if clients:
