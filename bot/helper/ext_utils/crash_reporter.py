...
class _ErrorTriggerHandler(Handler):
    ...
    def emit(self, record):
        ...
        try:
            ...
            # Routine LOGGER.error() calls are already handled by the code that
            # logged them (e.g. a per-task failure that gets caught and skipped) -
            # only genuinely unhandled crashes (below) should page the owner.
            bot_loop.create_task(_post_report(payload, notify_owner=False))
        finally:
            _sending_report = False
...
def send_unhandled_exception(exc_type, exc_value, exc_traceback):
    ...
    # A genuinely unhandled exception (sys.excepthook) - this is a real crash.
    bot_loop.create_task(_post_report(payload, notify_owner=True))
...
def send_async_exception(context):
    ...
    # An uncaught exception in an asyncio Task - also a genuine crash.
    bot_loop.create_task(_post_report(payload, notify_owner=True))
...
_owner_notify_last_sent = {}
_OWNER_NOTIFY_DEBOUNCE = 300


async def _notify_owner(payload):
    try:
        if not Config.OWNER_ID or TgClient.bot is None:
            return
        key = payload.get("exception", "")
        now = time()
        if now - _owner_notify_last_sent.get(key, 0) < _OWNER_NOTIFY_DEBOUNCE:
            return
        _owner_notify_last_sent[key] = now
        tb = payload.get("traceback", "")
        if len(tb) > 3000:
            tb = tb[:3000] + "\n... (truncated)"
        text = (
            f"⚠️ <b>Bot crash detected</b>\n"
            f"<b>Exception:</b> <code>{_esc_html(payload.get('exception', 'unknown'))}</code>\n"
            f"<b>Time:</b> {_esc_html(payload.get('timestamp', ''))}\n\n"
            f"<pre>{_esc_html(tb)}</pre>"
        )
        await TgClient.bot.send_message(Config.OWNER_ID, text)
    except Exception as e:
        LOGGER.warning(f"Failed to notify owner of crash: {e}")


async def _post_report(payload, notify_owner=False):
    if notify_owner:
        bot_loop.create_task(_notify_owner(payload))
    try:
