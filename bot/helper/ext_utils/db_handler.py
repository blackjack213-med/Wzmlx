...
class DbManager:
    ...
    async def get_topic_clone_job(self, source_chat_id):
        if self._return:
            return None
        return await self.db.topic_clone[_part()].find_one({"_id": source_chat_id})

    async def save_topic_clone_job(self, source_chat_id, data):
        if self._return:
            return
        await self.db.topic_clone[_part()].update_one(
            {"_id": source_chat_id}, {"$set": data}, upsert=True
        )

    async def mark_topic_clone_submitted(self, source_chat_id, topic_key, msg_id):
        if self._return:
            return
        await self.db.topic_clone[_part()].update_one(
            {"_id": source_chat_id},
            {"$addToSet": {f"submitted.{topic_key}": msg_id}},
            upsert=True,
        )
