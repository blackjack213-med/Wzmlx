...
class TelegramDownloadHelper:
    ...
    def _personal_client(self):
        return TgClient.personal_users.get(self._listener.user_id)
    ...
    async def _on_download_progress(self, current, _):
        if self._listener.is_cancelled:
            ...
            elif self.session == "personal":
                if personal := self._personal_client():
                    personal.stop_transmission()
        ...
        try:
            ...
            if self._hyper_dl:
                try:
                    ...
                    personal = (
                        self._personal_client()
                        if self.session == "personal"
                        else None
                    )
                    if personal is not None:
                        try:
                            personal_message = await personal.get_messages(
                                chat_id=message.chat.id, message_ids=message.id
                            )
                            download = await personal_message.download(
                    ...
                    elif self._listener.transmission_mode in ("user", "both"):
                        try:
