...
from re import IGNORECASE, findall as re_findall, match as re_match, sub as re_sub
...
def _promo_username_whitelist():
    if not Config.PROMO_TEXT:
        return set()
    return {u.lower() for u in re_findall(r"@(\w+)", Config.PROMO_TEXT)}


def _clean_precaption(text, whitelist=None):
    if not text:
        return text
    whitelist = whitelist or set()
    text = re_sub(r"https?://\S+", "", text)
    text = re_sub(r"(?:^|(?<=\s))t\.me/\S+", "", text)
    text = re_sub(
        r"@(\w+)", lambda m: m.group(0) if m.group(1).lower() in whitelist else "", text
    )
    text = re_sub(
        r"\b[\w-]+\.(?:com|net|org|io|co|in|me|tv|info|xyz|live|us|uk)\b",
        "",
        text,
        flags=IGNORECASE,
    )
    text = re_sub(r"[ \t]+\n", "\n", text)
    text = re_sub(r"[ \t]{2,}", " ", text)
    text = re_sub(r"\n{3,}", "\n\n", text)
    return text.strip()
...
class TelegramUploader:
    ...
    async def _prepare_file(self, pre_file_, dirpath):
        ...
        if lcaption:
            ...
            promo_whitelist = _promo_username_whitelist()
            cap_mono = parts[0].format(
                filename=_clean_precaption(cap_file_, promo_whitelist),
                ...
                precaption=_clean_precaption(
                    self._listener.file_details.get("caption", ""), promo_whitelist
                ),
            )
        ...
        if Config.PROMO_ENABLED and Config.PROMO_TEXT:
            cap_mono = (
                f"{cap_mono}\n\n{Config.PROMO_TEXT}" if cap_mono else Config.PROMO_TEXT
            )
