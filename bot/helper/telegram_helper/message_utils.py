...
async def get_tg_link_message(link, personal_client=None):
    message = None
    links = []
    if link.startswith(
        (
        ...
        if not TgClient.user and personal_client is None:
    ...
    if personal_client is not None:
        try:
            personal_message = await personal_client.get_messages(
                chat_id=chat, message_ids=msg_id
            )
            if not personal_message.empty:
                return (links, "personal") if links else (personal_message, "personal")
        except Exception:
            pass  # fall through to the shared USER_SESSION_STRING, then error out
