...
class Mirror(TaskListener):
    ...
    async def new_event(self):
        ...
        if is_telegram_link(self.link):
            ...
            personal_session = self.user_dict.get("USER_TG_SESSION")
            if personal_session:
                try:
                    personal_client = await TgClient.get_personal_client(
                        self.user_id, personal_session
                    )
                except Exception as e:
                    LOGGER.warning(
                        f"Personal session for user {self.user_id} failed to start: {e}"
                    )
            try:
                reply_to, session = await get_tg_link_message(
                    self.link, personal_client=personal_client
                )
            except Exception as e:
